...
namespace graphics {
  ...
  bool Window::init() {
	if (!glfwInit()) {
	  ...
	  utils::Logger::getInstance().error("[WindowInit]", "Could not initialize GLFW Window");
	  return false;
	}
	...
	if (!m_Window) {
	  utils::Logger::getInstance().error("[WindowInit]", "Could not create GLFW Window");
	...
	if (glewInit() != GLEW_OK) {
	  ...
	  utils::Logger::getInstance().error("[GlewInit]", "Could not initialize GLEW");
	  return 0;
	}
  ...
  bool Window::isKeyPressed(unsigned int keycode) {
	if (keycode >= MAX_KEYS) {
	  utils::Logger::getInstance().error("[Input]", "Key checked is out of bounds.");
	  return false;
	}
	else {
  ...
  bool Window::isMouseButtonPressed(unsigned int code) {
	if (code >= MAX_BUTTONS) {
	  utils::Logger::getInstance().error("[Input]", "Mouse button checked is out of bounds.");
	  return false;
	}
	else {
