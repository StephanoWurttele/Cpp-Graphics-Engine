  ...
  void Mesh::LoadData(bool interleaved) {
	// Check for possible mesh initialization errors
	{
	  ...
	  if (m_UVs.size() != 0 && m_UVs.size() != vertexCount)
		utils::Logger::getInstance().error("[MeshCreation]", "Mesh UV count doesn't match the vertex count.");
	  if (m_Normals.size() != 0 && m_Normals.size() != vertexCount)
		utils::Logger::getInstance().error("[MeshCreation]", "Mesh Normal count doesn't match the vertex count.");
	  if (m_Tangents.size() != 0 && m_Tangents.size() != vertexCount)
		utils::Logger::getInstance().error("[MeshCreation]", "Mesh Tangent count doesn't match the vertex count.");
	  if (m_Bitangents.size() != 0 && m_Bitangents.size() != vertexCount)
		utils::Logger::getInstance().error("[MeshCreation]", "Mesh Bitangent count doesn't match the vertex count.");
	}
